...
func GetFilteredProducts(c *gin.Context) {
	...
	if name := c.Query("name"); name != "" {
		filters = append(filters, "P.NAME ILIKE $"+fmt.Sprint(argID))
		args = append(args, "%"+name+"%")
		argID++
	}

	if minPrice := c.Query("min_price"); minPrice != "" {
		filters = append(filters, "P.BASE_PRICE >= $"+fmt.Sprint(argID))
	...
	query := "SELECT DISTINCT ON (V.ID) P.ID AS PRODUCT_ID, P.NAME AS PRODUCT_NAME, P.DESCRIPTION, P.BASE_PRICE, P.SKU AS PRODUCT_SKU, P.INVENTORY_QUANTITY AS PRODUCT_INVENTORY, P.HAS_VARIANTS, P.TAGS, P.RATING AS PRODUCT_RATING, P.VERIFIED_PURCHASES, P.DISCOUNT, V.ID AS VARIANT_ID, V.SKU AS VARIANT_SKU, V.PRICE AS VARIANT_PRICE, V.COLOR AS VARIANT_COLOR, V.SIZE AS VARIANT_SIZE, V.INVENTORY_QUANTITY AS VARIANT_INVENTORY, IMG.ID AS IMAGE_ID, IMG.URL AS IMAGE_URL, IMG.THUMBNAIL_URL, IMG.TYPE AS IMAGE_TYPE FROM PRODUCTS P LEFT JOIN PRODUCT_VARIANTS V ON V.PRODUCT_ID = P.ID LEFT JOIN PRODUCT_IMAGES IMG ON IMG.PRODUCT_ID = P.ID OR (IMG.VARIANT_ID = V.ID AND P.HAS_VARIANTS = TRUE)"
	
	if len(filters) > 0 {
		query += " WHERE " + strings.Join(filters, " AND ")
	}

	query += " ORDER BY V.ID, IMG.POSITION;"
